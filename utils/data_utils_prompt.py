...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, model_type, model_name, max_seq_length):
    ...
    def convert_examples_to_features(examples, label_list, max_seq_length,
        ...
        for ex_index, example in enumerate(tqdm(examples)):
            ...
            for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
                ...
                prompt0 = 'The question is'
                prompt_ids0 = tokenizer.tokenize(prompt0)
                prompt1 = "The candidate answer is"
                prompt_ids = tokenizer.tokenize(prompt1)
                prompt2 = "This means <mask>"  #
                prompt2_ids = tokenizer.tokenize(prompt2)
                ...
                tokens = prompt_ids0 + tokens_a + prompt_ids
                if sep_token_extra:
                ...
                if tokens_b:
                    ...
                    tokens += tokens_b + prompt2_ids
                    # tokens += tokens_b
                    segment_ids += [sequence_b_segment_id] * (len(tokens_b) + len(prompt2_ids))
                ...
                if len(input_ids) > max_seq_length:
                    length_delete = len(input_ids) - max_seq_length
                    delete_random_idx = [x for x in range(50, 1000, 10)]
                    delete_random_idx = delete_random_idx[:length_delete]
                    for idx in delete_random_idx:
                        input_ids = input_ids[:idx] + input_ids[idx+1:]
                        output_mask = output_mask[:idx] + output_mask[idx+1:]
                        input_mask = input_mask[:idx] + input_mask[idx+1:]
                        segment_ids = segment_ids[:idx] + segment_ids[idx+1:]
    ...
    model_name = '/your path/roberta-large/'
    # model_name = '/your path/SapBERT-from-PubMedBERT-fulltext/'  # for medqa
