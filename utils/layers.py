...
class MultiheadAttPoolLayer(nn.Module):
    ...
    def forward(self, q, k, mask=None):
        ...
        qs = qs.permute(1, 0, 2).contiguous().view(n_head * bs, d_k)  # [20, 100]
        ks = ks.permute(2, 0, 1, 3).contiguous().view(n_head * bs, len_k, d_k)  # [20,200, 100]
        vs = vs.permute(2, 0, 1, 3).contiguous().view(n_head * bs, len_k, d_v)  # [20,200, 100]
...
class SelfAttLayer(nn.Module):
    ...
    def __init__(self, mini_bs, n_head, d_q_original, d_k_original, num_choice, dropout=0.1):
        super().__init__()
        assert d_k_original % n_head == 0  # make sure the outpute dimension equals to d_k_origin
        self.mini_bs = mini_bs  # 2
        self.len_choice = num_choice
        self.n_head = n_head
        self.d_k = d_k_original // n_head  # 512
        self.d_v = d_k_original // n_head  # 512

        self.w_qs = nn.Linear(d_q_original, n_head * self.d_k)  # 1024 sent_dim, 2*512
        self.w_ks = nn.Linear(d_k_original, n_head * self.d_k)  # 1024 concept_dim, 2*512
        self.w_vs = nn.Linear(d_k_original, n_head * self.d_v)  # 1024 concept_dim, 2*512
        # self.LN = LayerNorm(d_k_original, eps=1e-12)

        nn.init.normal_(self.w_qs.weight, mean=0, std=np.sqrt(2.0 / (d_q_original + self.d_k)))
        nn.init.normal_(self.w_ks.weight, mean=0, std=np.sqrt(2.0 / (d_k_original + self.d_k)))
        nn.init.normal_(self.w_vs.weight, mean=0, std=np.sqrt(2.0 / (d_k_original + self.d_v)))
        # MatrixVectorScaledDotProductAttention
        self.attention = SelfMatrixVectorScaledDotProductAttention(temperature=np.power(self.d_k, 0.5))
        # self.dropout = nn.Dropout(dropout)

    def forward(self, q, k, mask=None):
        """
        q: tensor of shape (b, d_q_original)s
        k: tensor of shape (b, d_k_original)
        mask: tensor of shape (b, l) (optional, default None)
        returns: tensor of shape (b, n*d_v)
        """
        mini_bs, len_choice, n_head, d_k, d_v = self.mini_bs, self.len_choice, self.n_head, self.d_k, self.d_v  # d_k=d_v=d_k_original//n_head

        bs, _ = q.size()
        bs, _ = k.size()

        # RuntimeError: shape '[2, 5, 2, 512]' is invalid for input of size 5120
        qs = self.w_qs(q).view(-1, len_choice, n_head, d_k)  # (b, nc, n, dk)
        ks = self.w_ks(k).view(-1, len_choice, n_head, d_k)  # (b, nc, n, dk)
        vs = self.w_vs(k).view(-1, len_choice, n_head, d_k)
        # vs = self.w_vs(k).view(-1, len_choice, n_head, d_v)  # (b, nc, n, dk)

        qs = qs.permute(2, 0, 1, 3).contiguous().view(-1, len_choice, d_k)
        ks = ks.permute(2, 0, 1, 3).contiguous().view(-1, len_choice, d_k)
        vs = vs.permute(2, 0, 1, 3).contiguous().view(-1, len_choice, d_v)

        if mask is not None:
            mask = mask.repeat(n_head, 1)
        output, attn = self.attention(qs, ks, vs, mask=mask)  # mini*head, len_c, d_k

        output = output.view(n_head, bs, d_v)
        output = output.permute(1, 0, 2).contiguous().view(bs, n_head * d_v)  # (b, n*dv)
        # output = self.dropout(output)
        output = output + k  # short-cut
        return output, attn


class SelfMatrixVectorScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        """
        q: tensor of shape (n*b, len_c, d_k)
        k: tensor of shape (n*b, len_c, d_k)
        v: tensor of shape (n*b, len_c, d_v)
        mask: tensor of shape (n*b, l)
        returns: tensor of shape (n*b, d_v), tensor of shape(n*b, l)
        """
        attn = torch.matmul(q, k.transpose(-1, -2)) # (n*b, len_c, len_c)
        # attn = (q.unsqueeze(1) * k).sum(2)  # (n*b, l)
        attn = attn / self.temperature
        if mask is not None:
            attn = attn.masked_fill(mask, -np.inf)
        attn = self.softmax(attn) # (n*b, len_c, len_c)
        attn = self.dropout(attn) # (n*b, len_c, len_c)
        output = torch.matmul(attn, v) # (n*b, len_c, len_c)@(n*b, len_c, d_k)
        return output, attn
