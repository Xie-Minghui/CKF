...
from utils.data_utils_prompt import *
from utils.layers import *
import torch.nn.functional as F


class CAFE_Message_Passing(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, input_size, hidden_size, output_size, sent_dim,
        ...
        self.k = k
        self.gnn_layers = nn.ModuleList([GATConvE(args, hidden_size, n_ntype, n_etype, self.edge_encoder, _) for _ in range(k)])
        ...
        self.film_generator = nn.Linear(sent_dim, 2 * hidden_size)

    def mp_helper(self, _X, edge_index, edge_type, _node_type, _node_feature_extra, node_batch, sent_vecs, node_scores_flat, sent_vecs_notf):
        bs = sent_vecs.size(0)  # 10
        num_node = _X.size(0)//bs  # 200
        Z_index = [num_node*i for i in range(bs)]  # [0, 200, 400, ...,1800]

        film_vector_origin = self.film_generator(sent_vecs_notf)  # [10, 400]
        film_vector = film_vector_origin[node_batch]  # [2000, 400]
        film_vector = film_vector.view(film_vector.size(0), -1, 2)  # [2000,2000,2]
        gamma, beta = film_vector[:, :, 0].squeeze(), film_vector[:, :, 1].squeeze()
        _X = gamma * _X.clone() + beta  # FiLM transformation [2000,200]

        for _ in range(self.k):
            Z_node = _X[Z_index, :]  # [10, 200]
            _X = self.gnn_layers[_](_X, edge_index, edge_type, _node_type, _node_feature_extra, node_batch, sent_vecs, node_scores_flat, Z_node, film_vector_origin)
            _X = self.activation(_X)
            _X = F.dropout(_X, self.dropout_rate, training = self.training)
        return _X

    def forward(self, H, A, node_type, node_score, node_batch, sent_vecs, sent_vecs_notf, cache_output=False):
        """
        ...
        node_scores_flat = node_score.clone().detach().squeeze()  # (10, 32 ,1)
        node_scores_flat = node_scores_flat.view(node_scores_flat.size(0) * node_scores_flat.size(1)).unsqueeze(1)  #
        _X = self.mp_helper(_X, edge_index, edge_type, _node_type, _node_feature_extra, node_batch, sent_vecs,
                            node_scores_flat,sent_vecs_notf)
...
class CAFE(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim,
        ...
        self.gnn = CAFE_Message_Passing(args, k=k, n_ntype=n_ntype, n_etype=n_etype,
                                        input_size=concept_dim, hidden_size=concept_dim, output_size=concept_dim, sent_dim=sent_dim, dropout=p_gnn)

        self.pooler = MultiheadAttPoolLayer(n_attention_head, sent_dim, concept_dim)  # n_attention_head设置为1
        # self.pooler = MultiheadAttPoolLayer(n_attention_head, concept_dim, concept_dim)  # n_attention_head设置为1
    ...
    def forward(self, sent_vecs, concept_ids, node_type_ids, node_scores, adj_lengths, adj, node_batch, emb_data=None, cache_output=False):
        """
        ...
        """
        sent_vecs_transformed = self.svec2nvec(sent_vecs)
        gnn_input0 = self.activation(sent_vecs_transformed).unsqueeze(1) #(batch_size, 1, dim_node)
        ...
        gnn_input = self.dropout_e(torch.cat([gnn_input0, gnn_input1], dim=1)) #(batch_size, n_node, dim_node) # [10, 200, 200]
        ...
        gnn_output = self.gnn(gnn_input, adj, node_type_ids, node_scores, node_batch, sent_vecs_transformed, sent_vecs)

        Z_vecs = gnn_output[:, 0]  # (batch_size, dim_node)
        
        mask = torch.arange(node_type_ids.size(1), device=node_type_ids.device) >= adj_lengths.unsqueeze(1) #1 means masked ou
        ...
        return graph_vecs, Z_vecs

class LM_CAFE(nn.Module):
    def __init__(self, args, model_name, k, n_ntype, n_etype,
        ...
        self.decoder = CAFE(args, k, n_ntype, n_etype, self.encoder.sent_dim,
                                        n_concept, concept_dim, concept_in_dim, n_attention_head,
                                        fc_dim, n_fc_layer, p_emb, p_gnn, p_fc,
                                        pretrained_concept_emb=pretrained_concept_emb, freeze_ent_emb=freeze_ent_emb,
                                        init_range=init_range)
        self.fc = MLP(self.encoder.sent_dim + concept_dim + concept_dim, fc_dim, 1, n_fc_layer, p_fc, layer_norm=True)
        self.dropout_fc = nn.Dropout(p_fc)
        self.graph2lm_att = MultiheadAttPoolLayer(n_head=4, d_q_original=concept_dim, d_k_original=self.encoder.sent_dim)
    ...
    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False):
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs
        ...
        node_batch = self.get_node_batch(node_type_ids).to(node_type_ids.device)

        sent_vecs, all_hidden_states = self.encoder(*lm_inputs, layer_id=layer_id)

        attention_mask = lm_inputs[1]
        loc_mask_token = torch.sum(attention_mask, dim=1) - 1
        mask_token = []
        for i in range(loc_mask_token.shape[0]):
            mask_token.append(all_hidden_states[-1][i, loc_mask_token[i]])
        mask_token = torch.stack(mask_token, dim=0)
        sent_vecs = mask_token

        graph_vecs, Z_vecs = self.decoder(sent_vecs.to(node_type_ids.device),
                                    concept_ids,
                                    node_type_ids, node_scores, adj_lengths, adj, node_batch,
                                    emb_data=None, cache_output=cache_output)
        attention_mask = lm_inputs[1]
        sent_vecs_new, att_out = self.graph2lm_att(graph_vecs, all_hidden_states[-1], attention_mask==0)  #  all_hidden_states[-1], attention_mask, Z_vecs
        concat = self.dropout_fc(torch.cat((sent_vecs, graph_vecs, Z_vecs), 1)) 
        concat1 = self.dropout_fc(torch.cat((sent_vecs_new, graph_vecs, Z_vecs), 1))  

        logits1 = self.fc(concat)
        logits2 = self.fc(concat1)
    ...
    def get_node_batch(self, node_type_ids):
        _batch_size, _n_nodes = node_type_ids.size()
        n_node_total = _batch_size * _n_nodes

        node_batch = [i//_n_nodes for i in range(n_node_total)]
        node_batch = torch.tensor(node_batch, dtype=torch.int64)
...
class LM_CAFE_DataLoader(object):
...
class GATConvE(MessagePassing):
    ...
    """
    def __init__(self, args, emb_dim, n_ntype, n_etype, edge_encoder, layer_id, head_count=4, aggr="add"):
        ...
        self.linear_query2 = nn.Linear(3*emb_dim, head_count * self.dim_per_head)
    ...
    def forward(self, x, edge_index, edge_type, node_type, node_feature_extra, node_batch, sent_vecs, node_scores_flat, Z_node, film_vector_origin, return_attention_weights=False):
        # x: [N, emb_dim]
        # edge_index: [2, E]
        ...
        if self.layer_id == 0:
            film_vector_edge = film_vector_origin[edge_batch]
            film_vector_edge = film_vector_edge.view(film_vector_edge.size(0), -1, 2) 
            gamma, beta = film_vector_edge[:, :, 0].squeeze(), film_vector_edge[:, :, 1].squeeze()
            edge_embeddings = gamma * (edge_embeddings.clone() + x[edge_batch]) + beta

        x = torch.cat([x, node_feature_extra], dim=1)
        x = (x, x)
        aggr_out = self.propagate(edge_index, x=x, edge_attr=edge_embeddings, sent_vecs=Z_node[edge_batch]) #[N, emb_dim]
    ...
    def message(self, edge_index, x_i, x_j, edge_attr, sent_vecs): #i: tgt, j:src
        ...
        query = self.linear_query2(torch.cat([x_j, sent_vecs], dim=1)).view(-1, self.head_count, self.dim_per_head) #[E, heads, _dim]
        ...
        out = out.view(-1, self.head_count * self.dim_per_head)
        # [E, emb_dim]
        return out
