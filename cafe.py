...
from modeling.modeling_cafe import *
from utils.optimization_utils import OPTIMIZER_CLASSES
from utils.parser_utils import *

import setproctitle

setproctitle.setproctitle("CAFE")
...
def main():
    ...
    parser.add_argument('--load_model_path', default='your path')
    ...
    parser.add_argument('--att_head_num', default=4, type=int, help='number of attention heads')
...
def train(args):
    ...
    if True:
        ...
        dataset = LM_CAFE_DataLoader(args, args.train_statements, args.train_adj,
        ...
        model = LM_CAFE(args, args.encoder, k=args.k, n_ntype=4, n_etype=args.num_relation, n_concept=concept_num,
    ...
    # try:
        for epoch_id in range(args.n_epochs):
            ...
            total_acc = []
            count = 0
            save_dir = os.path.join(args.save_dir, 'method_{}/'.format(METHOD))
            if not os.path.exists(save_dir):
                os.mkdir(save_dir)
            preds_path_json = os.path.join(save_dir, 'test_e{}_preds.json'.format(epoch_id))
            with open(preds_path_json, 'w') as f_preds:
                with torch.no_grad():
                    for qids, labels, *input_data in tqdm(eval_set):
                        count += 1
                        ...
                        for i, (qid, label, pred, _preds_ranked, cids, ntype, edges, etype, logit) in enumerate(
                                zip(qids, labels, predictions, preds_ranked, concept_ids, node_type_ids, edge_index,
                                    edge_type, logits)):
                            acc = int(pred.item() == label.item())
                            total_acc.append(acc)
                            if acc == 0:
                                wrong_example = {'question id': qid, 'right answer': {'choices': chr(ord('A') + label.item()),
                                                                       'logit': round(logit[label.item()].item(), 3)},
                                                 'predicted answer': {'choices': chr(ord('A') + pred.item()),
                                                          'logit': round(logit[pred.item()].item(), 3)},
                                                 'answer order': {'choices': [chr(ord('A') + x.item()) for x in _preds_ranked],
                                                          'logit': [round(logits[i,x.item()].item(), 3) for x in _preds_ranked]}}
                                json_str = json.dumps(wrong_example, ensure_ascii=False)  # indent=4
                                f_preds.write(json_str)
                                f_preds.write('\n')
            ...
            if args.save_model and dev_acc >= best_dev_acc:
...
def eval_detail(args):
    ...
    model = LM_CAFE(old_args, old_args.encoder, k=old_args.k, n_ntype=4, n_etype=old_args.num_relation, n_concept=concept_num,
...
def inference(args):
    ...
    model_state_dict, old_args = torch.load(model_path, map_location=torch.device('cuda'))
    model = LM_CAFE(old_args, old_args.encoder, k=old_args.k, n_ntype=4, n_etype=old_args.num_relation, n_concept=concept_num,
                               concept_dim=old_args.gnn_dim,
                               concept_in_dim=concept_dim,
                               n_attention_head=old_args.att_head_num, fc_dim=old_args.fc_dim, n_fc_layer=old_args.fc_layer_num,
    ...
    if not save_test_preds:
        ...
        dt = datetime.datetime.today().strftime('%Y%m%d%H%M%S')
        save_dir = 'you path'
        preds_path = os.path.join(save_dir, 'test_preds_{}.csv'.format(dt))
        with open(preds_path, 'w') as f_preds:
            with torch.no_grad():
                for qids, labels, *input_data in tqdm(eval_set):
                    count += 1
...
def inference_json(args):
    ...
    if not save_test_preds:
        ...
        dt = datetime.datetime.today().strftime('%Y%m%d%H%M%S')
        save_dir = 'your path/'
        # save_dir = os.path.join(args.save_dir, 'inference/')
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        preds_path_json = os.path.join(save_dir, 'inference_test.json')
        with open(preds_path_json, 'w') as f_preds:
            with torch.no_grad():
                for qids, labels, *input_data in tqdm(eval_set):
                    count += 1
                    ...
                    for i, (qid, label, pred, _preds_ranked, cids, ntype, edges, etype) in enumerate(zip(qids, labels, predictions, preds_ranked, concept_ids, node_type_ids, edge_index, edge_type)):
                        # acc = int(pred.item()==label.item())
                        # print ('{},{}'.format(qid, chr(ord('A') + pred.item())), file=f_preds)
                        example = {"id": qid, "answerKey": chr(ord('A') + pred.item())}
                        # f_preds.flush()
                        # total_acc.append(acc)
